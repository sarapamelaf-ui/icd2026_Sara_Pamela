...
# Interpretação:
# Com os parametros usados, a média simulada fica proxima de 42 conversões
# por semana, que é o valor esperado téorico 600*(0.07).
# das semanas. Portanto, uma unica semana abaixo desse limite ainda parece uma 
# variação plausivel do processo, não um sinal forte de problema por si so.
# Semanas com 55 conversões ou mias são menos frequentes (cerca de 3%), mas 
# também podem ocorrer dentro da variabiliudade esperada.
# O resultado ficaria mais preocupante se o baixo desempenho se repetisse por
# varias semanas ou viesse acompanhado de evidencias externas.
...
# Mostra os 10 promeiros valores simulados
head(clientes, 10)
...
# So calcula desvio padrão se tiver calculado a variancia
...
excedente_medio_atual <- mean(
  ifelse(clientes > capacidade, clientes - capacidade, 0)
)
...
excedente_medio <- c(
  mean(ifelse(clientes > 20, clientes - 20, 0)),
  mean(ifelse(clientes > 22, clientes - 22, 0)),
  mean(ifelse(clientes > 25, clientes - 25, 0))
)

# Capacidade ociosa, considerando todas as horas simuladas.
ociosidade_media <- c(
  mean(ifelse(clientes < 20, 20 - clientes, 0)),
  mean(ifelse(clientes < 22, 22 - clientes, 0)),
  mean(ifelse(clientes < 25, 25 - clientes, 0))
)
...
# A capacidade de 20 clientes por hora tem menor ociocidade, mas gera saturação
# elevada: cerca de 27% das horas por reduz a saturação, mas ainda deixa cerca de 15% das horas simuladas acima da capacidade
# A capacidade de 25  clientes por hora eduz a saturação de 95% da simulação,
# Alem disso, 25 clientes por hora concide com o quantil de 95% da simulação,
# ou seja, atende aproximadamente 95% das horas simuladas.
# Se a prioridade for reduzur espera e preservar a qualidade do atendimento no pico, a capacidade de 25 parece mais adequada. Se o custo da ociosidade pesar 
#@ mais e a unidade aceitar mais saturação, a capacidade atual de 22 também pode
# ser defendida. A simulação não decide soxinha: ela qantifica o trade-off.
